...
export const Card = forwardRef<HTMLDivElement, CardProps>(({ customClass, ...rest }, ref) => (
  <div
    ref={ref}
    {...rest}
    className={`absolute top-1/2 left-1/2 rounded-xl border border-white/10 bg-neutral-900 text-white shadow-xl [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden] ${customClass ?? ""} ${rest.className ?? ""}`.trim()}
  />
));
...
const CardSwap = forwardRef<CardSwapHandle, CardSwapProps>(function CardSwap(
  ...
  if (autoPlay) {
    ...
    if (pauseOnHover) {
      ...
      const resume = () => {
        tlRef.current?.play();
        if (autoPlay) intervalRef.current = window.setInterval(swap, delay);
      };
  ...
  next: () => {
      ...
      const total = refs.length;
      // Place to current slots to avoid drift
      refs.forEach((r, i) => r.current && placeNow(r.current, makeSlot(i, cardDistance, verticalDistance, total), skewAmount));
  ...
  onActiveChange?.(last);
      rest.forEach((idx, i) => {
        ...
        tl.to(el, { x: slot.x, y: slot.y, z: slot.z, duration: cfg.durMove * 0.8, ease: cfg.ease }, i === 0 ? 0 : "<0.05");
  ...
  return (
    <div
      ref={container}
      className={`absolute bottom-0 right-0 translate-x-[5%] translate-y-[20%] origin-bottom-right perspective-[900px] overflow-visible max-lg:translate-x-[20%] max-lg:translate-y-[20%] max-lg:scale-[0.85] max-sm:scale-[0.65] ${className ?? ""}`}
      style={{ width, height }}
    >
      {rendered}
    </div>
